...
class Dissasembler:
    ...
    def __init__(self,text,st,rc,pcstart = 0):
        self.text = text
        self.st = st
        self.rc = rc
    ...
    def output(self):
        ...
        for I in self.Instructions:
            if (self.pc + self.pcstart) in self.st.keys():
                lines.append(self._output_sym(self.st[self.pc + self.pcstart]))
    ...
    def _decode_RI(self,opcode):
        rg = regLookup[self.text[self.pc + 1] >> 4]
    ...
    def _decode_register_pair(self):
        ...
        SRC = regLookup[(pairbyte >> 4) & 0xf]
        DEST = regLookup[(pairbyte & 0xf)]
